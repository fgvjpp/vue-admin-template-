      ...
      <el-col :span="13" style="padding-right:26px">
        ...
        <el-card class="box-card">
          ...
          <el-row type="flex" justify="end">
            <el-select v-model="currentyear" style="width:100px;" size="mini" placeholder="" @change="getNewDates">
              <el-option v-for="item in 11" :key="item" :label="(dateYear + item) + '年'" :value="dateYear + item" />
            </el-select>
            <el-select v-model="currentMonth" style="width:100px;" size="mini" placeholder="" @change="getNewDates">
              <el-option v-for="item in 12" :key="item" :label="item + '月'" :value="item" />
            </el-select>
          </el-row>
          <el-calendar :key="$store.state.calendarApply" v-model="Newdates" class="calendar">
            <template #dateCell="{data}">
              <span>{{ getDay(data.day) }}</span>
              <div v-if="checkDate(data.day)" class="rest">休</div>
            </template>
          </el-calendar>
        </el-card>
        <!-- 公告 -->
...
export default {
  ...
  methods: {
    ...
    getDay(day) {
      const newDay = day.slice(8)
      return newDay[0].indexOf('0') > -1 ? newDay[1] : newDay
    },
    checkDate(day) {
