  ...
  <div class="user-info">
    ...
    <el-form v-loading="loading" label-width="220px">
      <el-row type="flex" justify="end">
        <el-button size="small" type="warning" @click="$router.push(`/employees/print/${userId}?type=personal`)">
      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <uoload-img ref="upload" :userimg="userInfo.staffPhoto" @userImg="userInfo.staffPhoto = $event" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form v-loading="loading" label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <uoload-img :userimg="formData.staffPhoto" @userImg="formData.staffPhoto = $event" />
        </el-form-item>
      ...
      <div class="block">
        ...
        <el-row class="inline-info" type="flex" justify="center">
          <el-col :span="12">
            <el-button type="primary" @click="saveEmployeesInfoAll">保存更新</el-button>
            <el-button @click="$router.back()">返回</el-button>
          </el-col>
        </el-row>
      </div>
...
import { getEmployeesInfo, saveEmployeesInfoAll, saveEmployeesInfo } from '@/api/employees'
import { getUserDetailById } from '@/api/user'
import uoloadImg from './uploadImg.vue'
...
export default {
  ...
  methods: {
    // 保存员工个人信息
    async saveEmployeesInfoAll() {
      if (!this.$refs.upload.progressShow) {
        this.loading = true
        await saveEmployeesInfoAll(this.formData)
        this.getEmployeesInfo()
        this.$message.success('保存个人信息成功')
      } else {
        this.$message.error('头像信息还没上传完成')
      }
    },
    // 保存员工基本信息
    async saveEmployeesInfo() {
      if (!this.$refs.upload.progressShow) {
        ...
        // this.userInfo.staffPhoto = 'http://tva3.sinaimg.cn/large/006APoFYly1g5uwieo18pg308c0784a1.gif'

        await saveEmployeesInfo(this.userInfo)
        this.getUserDetailById()
    ...
    async getUserDetailById() {
      const res2 = await getUserDetailById(this.userId)
