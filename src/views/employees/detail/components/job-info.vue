  ...
  <div class="job-info">
    ...
    <el-form v-loading="loading" label-width="220px">
      <el-row type="flex" justify="end">
        <el-button size="small" type="warning" @click="$router.push(`/employees/print/${userId}?type=Job`)">
...
import { getEmployeesJobInfo, saveEmployeesJobInfo, getSimpleUserInfo } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getEmployeesJobInfo() {
      const { data } = await getEmployeesJobInfo(this.userId)
      this.formData = data
      this.loading = false
    },
    // 保存岗位信息
