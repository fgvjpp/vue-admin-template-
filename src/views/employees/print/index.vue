    ...
    <div class="app-container">
      <el-card>
        <el-row type="flex">
          ...
          <el-button v-print="{id:'myPrint'}" size="small" type="warning">
...
<script>
import { getEmployeesInfo, getEmployeesJobInfo } from '@/api/employees'
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getEmployeesInfo() : this.getEmployeesJobInfo()
  },
  // 组件更新
  methods: {
    async getEmployeesInfo() {
      const res = await getEmployeesInfo(this.userId) // 获取个人基本信息
      this.formData = res.data
    },
    async getEmployeesJobInfo() {
      const userInfo = await getUserDetailById(this.userId)
      const jobInfo = await getEmployeesJobInfo(this.userId) // 获取岗位基本信息
      this.formData = { ...userInfo.data, ...jobInfo.data }
    }
  }
}
</script>
